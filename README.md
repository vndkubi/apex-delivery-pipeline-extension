...
2. Open the active artifact with **Open or Create Artifact**.
3. Run **Run Phase with Copilot** to use direct model execution or the scoped Copilot Chat handoff.
4. Use **Propose Artifact Update** when you want a reviewable diff instead of an immediate chat-driven rewrite.
5. Inspect **Open Developer Trace Panel** when you need the exact prompt, context set, execution path, or fallback reason.
6. Run **Configure Phase Profile** when a single phase needs different auto-submit, agent tag, or model-family defaults.
7. Run **Run Guided Autopilot** or **Run Fully Automatic Autopilot** from the Command Palette when the current phase policy allows it.
8. Use **Pause Guided Autopilot** or **Resume Guided Autopilot** when the flow requires human review or a later continuation.
9. Run **Mark Phase Passed** when the current artifact satisfies the gate and should advance the workflow.
