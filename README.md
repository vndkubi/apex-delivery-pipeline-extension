...
- Runs Guided Autopilot across autopilot-enabled phases by reusing the existing phase runner, verification checks, and status transitions.
...
9. Use **Run Guided Autopilot** when the workflow snapshot marks the current phase as autopilot-enabled and you want the extension to execute, verify, and advance phases until a manual stop point.
...
Guided Autopilot is phase-scoped. It only continues automatically when the snapshotted workflow marks the current phase with `autopilot.enabled: true`. If verification fails beyond the configured retry limit, or if the active artifact has unsaved manual edits and pause-on-manual-intervention is enabled, Guided Autopilot pauses and records the reason in workspace state plus the developer trace.
...
| Run Guided Autopilot | Reuses the phase runner across autopilot-enabled phases, pauses on failed verification or unsaved manual edits, and advances the workflow automatically until a manual stop point |
...
7. Use **Run Guided Autopilot** for workflow phases that are marked as autopilot-enabled when you want the extension to run, verify, and advance without manually triggering each phase.
8. Each phase run updates the artifact with the latest verification evidence when the extension can detect or is given build, test, and lint commands.
9. Use **Propose Artifact Update** if you want a developer-controlled diff-and-apply flow instead of going straight to Chat.
10. Inspect **Open Developer Trace Panel** when you need to understand what prompt was used, which files were in context, which path was chosen, why a fallback happened, and whether the current user role matched the phase role.
11. If you want a scoped follow-up inside the APEX participant, continue with `@apex`.
12. Mark the phase passed when the artifact satisfies the gate, or let Guided Autopilot advance it automatically when policy and verification allow.
